...
## Context-limit incident and fix

The first dual-v1 GW5 request was rejected by Jev with HTTP 400, `max_tokens_exceeded`. The release spending estimate did not enforce the separate per-request context limit. The question format is now `football-dual-v2`: player features use a labelled table, lower-priority recent detail/evidence is bounded, and only supplied reporting receives semantic questions. Conservative UTF-8 ceilings (20 KB state, 24 KB state plus longest question, 48 KB entire request) are checked before sending; these are distinct from token-based billing estimates. The statistical calculation retains the full numerical features locally. Published evidence lists only the excerpts actually sent.

The `Inspect prediction checkpoint` manual workflow reads release metadata without modifying storage. Its optional `replay_fixture` input explicitly sends one previously rejected fixture to Jev with retries disabled; it may cost credits, never publishes results, and is for operator diagnostics only. Leave that input blank for a read-only inspection. Private checkpoints now retain redacted provider error details. The history cache is saved even when prediction generation fails. Started gameweeks and uncertain requests remain held; this fix does not reopen GW5.
